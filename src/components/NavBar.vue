...
// Placeholder for user data (can be replaced with real authentication logic)
const currentUser = ref(null)

// Simulated authentication check
onMounted(() => {
  // Example of seeding users: a regular user and a stylist
  currentUser.value = {
    id: 1,
    name: 'Jane Doe',
    role: 'stylist' // 'stylist' or 'user'
  }
})
...
const isStylist = computed(() => currentUser.value?.role === 'stylist')
</script>

<template>
  <!-- Navbar is only visible if the user is authenticated -->
  <nav v-if="isAuthenticated" class="bg-white border-gray-200 dark:bg-gray-900 shadow-lg">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
      <!-- Brand and Logo -->
      <a href="#" class="flex items-center space-x-3">
        <img src="/Users/deezi/Falla/frontend/public/falla.jpg" class="h-8" alt="F" />
      ...
      <div class="hidden w-full md:block md:w-auto" id="navbar-default">
        <ul class="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700 items-center">
          ...
          <li>
            <a href="/warderobe" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white">
              Warderobe
            </a>
          </li>
        
          <!-- Clothing Suggestions always visible -->
          <li>
            <a href="/clothing-suggestions" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white">
